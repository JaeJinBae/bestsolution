...
.list_wrap > ul > .item:first-child, .list_wrap > ul > .item:nth-child(5){
	margin-left: 0px;
}
.list_wrap > ul > .item > a{
	position: relative;
...
.list_wrap > ul > li.item a:hover::before,
.list_wrap > ul > li.item a:hover::after{opacity:1;}
.list_wrap > ul > .item > a > b{
	position: absolute;
	bottom: 0;
	width: 100%;
	padding: 15px 0;
							...
							<c:choose>
							    ...
							    <c:otherwise>
							        <c:forEach var="item" items="${list}">
										<li class="item">
											<a href="${pageContext.request.contextPath}/menu03_01read${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}" style="background-image: url('${pageContext.request.contextPath}/resources/uploadPortfolio/${item.thumb_stored}');">
												<b>${item.title}</b>
											</a>
										</li>
									</c:forEach>
